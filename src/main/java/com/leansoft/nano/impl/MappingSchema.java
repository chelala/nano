...
class MappingSchema {
    ...
    private void buildRootElementSchema() {
        ...
        // Check if package annotation present and retain value
        ...
        if (type.isAnnotationPresent(RootElement.class)) {
            ...
            if (StringUtil.isEmpty(xre.name())) {
                ...
                if (type.isAnnotationPresent(Type.class)) {
                    final Type typeAnnotation = type.getAnnotation(Type.class);
                    if (!StringUtil.isEmpty(typeAnnotation.name())) {
                        rootElementSchema.setXmlName(typeAnnotation.name());
                    }
                }
            } else {
                rootElementSchema.setXmlName(xre.name());
            }
            String namespace = StringUtil.isEmpty(xre.namespace()) ? packageNamespace : xre.namespace();
            ...
            rootElementSchema.setNamespace(packageNamespace); // TODO: Recover
                                                              // NS from
            // annotation
        }
    }
    ...
    private String getPackageNamespace() {
        ...
        try {
            if (null == type.getClassLoader()) {
                return null;
            }
            final Class<?> packageInfoClass = type.getClassLoader().loadClass(type.getPackage().getName() + ".PackageInfo");
            if (packageInfoClass.isAnnotationPresent(Schema.class)) {
                final Schema schemaAnnotation = packageInfoClass.getAnnotation(Schema.class);
                if (!StringUtil.isEmpty(schemaAnnotation.namespace())) {
                    packageNamespace  = schemaAnnotation.namespace();
                }
            }
    ...
    private Map<String, Object> scanFieldSchema(Class<?> type) throws MappingException {
        ...
        for (Field field : fields) {
            ...
            if (field.isAnnotationPresent(Element.class)) {
                ...
                if (!Transformer.isTransformable(field.getType())) {
                    throw new MappingException("Attribute annotation can't annotate complex type field, " + "only primivte type or frequently used java type or enum type field is allowed, "
                ...
                // validation
                if (!Transformer.isTransformable(field.getType())) {
                    throw new MappingException("Value annotation can't annotate complex type field, " + "only primivte type or frequently used java type or enum type field is allowed, " + "field = "
                ...
                if (!TypeReflector.collectionAssignable(field.getType())) {
                    throw new MappingException("Current nano framework only supports java.util.List<T> as container of any type, " + "field = " + field.getName() + ", type = " + type.getName());
                }
        ...
        if (valueSchemaCount > 1) {
            throw new MappingException("Value annotation can't annotate more than one fields in same class," + " type = " + type.getName());
        }

        if (anyElementSchemaCount > 1) {
            throw new MappingException("AnyElement annotation can't annotate more than one fields in same class," + " type = " + type.getName());
        }

        if (valueSchemaCount == 1 && elementSchemaCount >= 1) {
            throw new MappingException("Value and Element annotations can't coexist in same class," + " type = " + type.getName());
        }
        ...
        if (TypeReflector.collectionAssignable(field.getType())) {
            ...
            if (!TypeReflector.isList(type)) {
                ...
                if (paramizedType == null) {
                    throw new MappingException("Can't get parameterized type of a List field, " + "Nano framework only supports collection field of List<T> type, and T must be a Nano bindable type, "
                            + "field = " + field.getName() + ", type = " + type.getName());
                } else {
