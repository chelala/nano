...
public class XmlPullWriter implements IWriter {
    ...
    protected void validate(Object source, Writer out) {
        ...
        if (Transformer.isPrimitive(source.getClass())) {
            throw new IllegalArgumentException(
                    "Can not write primitive or enum type object, "
    ...
    protected void writeObject(XmlSerializer serializer, Object source,
    ...
    private void writeAnyElements(XmlSerializer serializer, Object source,
    ...
    private void writeDomElement(XmlSerializer serializer, Element element)
            throws Exception {
        if (element == null)
            return; // be cautious
    ...
    private void writeAnyObject(XmlSerializer serializer, Object source)
            throws Exception {
        if (source == null)
            return; // be cautious
    ...
    private void writeAttributes(XmlSerializer serializer, Object source,
            MappingSchema ms) throws Exception {
        Map<String, AttributeSchema> field2AttributeSchemaMapping = ms
                .getField2AttributeSchemaMapping();
    ...
    private void writeValue(XmlSerializer serializer, Object source,
            MappingSchema ms) throws Exception {
    ...
    private void writeElements(XmlSerializer serializer, Object source,
            MappingSchema ms, String namespace) throws Exception {
    ...
    private void writeElementList(XmlSerializer serializer, Object source,
            ElementSchema es, String namespace) throws Exception {
        for (Object value : (List<?>) source) {
            this.writeElement(serializer, value, es, namespace);
        }
    }

    private void writeElement(XmlSerializer serializer, Object source,
            ElementSchema es, String namespace) throws Exception {
        Class<?> type = null;
        if (es.isList()) {
        ...
        if (!source.getClass().equals(type)) {
            ...
            final String sourceNamespace = ms.getRootElementSchema().getNamespace();
            String prefixAndValue = ms.getRootElementSchema().getXmlName();

            if (null != sourceNamespace) {
                final String prefix = serializer.getPrefix(sourceNamespace, true);

                if (!prefix.isEmpty()) {
                    prefixAndValue = prefix + ":" + prefixAndValue;
                }
            }
            serializer.attribute("http://www.w3.org/2001/XMLSchema-instance",
                    "type", prefixAndValue);
        }
        this.writeObject(serializer, source, typeNamespace);
        serializer.endTag(namespace, xmlName);
    }
